...
class main():
    ...
    def classify(self, train_df, test_df, LABELS, numOfLabels):
        '''
        ...
        # train_bodies_csv = os.path.join(data_folder, 'train_bodies.csv')
        # train_stance_csv = os.path.join(data_folder, 'train_stances.csv')
        # test_bodies_csv = os.path.join(data_folder, 'test_bodies.csv')
        # test_stances_csv = os.path.join(data_folder, 'competition_test_stances.csv')

        # "read in training sets"
        # train_bodies_df = pd.read_csv(r'' + train_bodies_csv, delimiter=',', header=0, sep='\t',
        #                               names=['body_id', 'article'])
        # train_stances_df = pd.read_csv(r'' + train_stance_csv, delimiter=',', header=0, sep='\t',
        ...
        # test_bodies_df = pd.read_csv(r'' + test_bodies_csv, delimiter=',', header=0, sep='\t',
        #                              names=['body_id', 'article'])
        # test_stances_df = pd.read_csv(r'' + test_stances_csv, delimiter=',', header=0, sep='\t',
        #                               names=['headline', 'body_id', 'stance'])
        ...
        #############################################################################
        ds = DataSet(preprocess=PREPROCESS, labelsFromDB=LABELS, train_df=train_df, test_df=test_df)
        ...
        fe = FeatureExtract(train_all, over_sampling=OVER_SAMPLING, set='train', class_format='trinary', labels=LABELS)
        ...
        fe_test = FeatureExtract(test_all, set='test', class_format='trinary', labels=LABELS)
        ...
        return self.NN_model(X_train_tri,y_train_tri,X_test,y_test)
    ...
    def NN_model(self, X, y, X_test, y_test):
        ...
        :return:
        '''
        num_of_classes=len(self.labels)
        y = np_utils.to_categorical(y, num_of_classes)
        y_true = y_test

...
class FNCData:
    ...
    def __init__(self, file):
        ...
        # Load data
        self.instances = self.dfInstance(file)
        bodies = self.dfBodies(file)
        ...
        self.bodies = {}
        # Process instances
        # add headlines to self.heads
        for instance in self.instances:
            if instance[headline] not in self.heads:
                head_id = len(self.heads)
                self.heads[instance[headline]] = head_id
            # if int(instance['Body ID']) not in self.bodies:
            #     self.bodies[int(instance['Body ID'])] = instance['articleBody']3
            # cast body id from string to int
            instance[bodyID] = int(instance[bodyID])

        # Process bodies
        # add bodies to self.bodies
        for body in bodies:
            self.bodies[int(body[bodyID])] = body[articleBody]
...
def pipeline_train(train, test, label_ref, lim_unigram):

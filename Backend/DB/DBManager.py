...
# db.insert_desc_model("LIU", "This model using 2 step approach to build the model. A conventional classifier SVM aims to perform a binary classification which predicts the stance of an onstance as related or unrelated. meanwhile, a 3-layer feed-forward neural network determines the stance. The neural network is using SoftMax activation function as the final layer. The final step is to combine the results generated by each classification model. Please notice that to make this algorithm a generic one we did some changes. For more details you are welcome to enter our git repository.")
